...
<script type="text/javascript">
	...
	$(function(){
		...
		// 保存新建线索
		$("#saveClueBtn").click(saveClue)

		// 刷新线索列表
		pageList(1,2)

		// 查询
		$("#searchBtn").click(searchClue)

		// 全选功能
		$("#selectAll").click(function (){
			$("input[name='selectClue']").prop("checked",this.checked)
		})

		$("#clueBody").on("click","input[name='selectClue']",function (){
			$("#selectAll").prop("checked",$("input[name='selectClue']").length==$("input[name='selectClue']:checked").length)
		})
	});

	// 查询功能
	function searchClue(){
		$("#hiddenFullname").val($.trim($("#searchFullname").val()));
		$("#hiddenCompany").val($.trim($("#searchCompany").val()));
		$("#hiddenPhone").val($.trim($("#searchPhone").val()));
		$("#hiddenSource").val($.trim($("#searchSource").val()));
		$("#hiddenOwner").val($.trim($("#searchOwner").val()));
		$("#hiddenMphone").val($.trim($("#searchMphone").val()));
		$("#hiddenState").val($.trim($("#searchState").val()));
    ...
    function pageList(pageNo,pageSize){
		$("#searchFullname").val($("#hiddenFullname").val());
		$("#searchCompany").val($("#hiddenCompany").val());
		$("#searchPhone").val($("#hiddenPhone").val());
		$("#searchSource").val($("#hiddenSource").val());
		$("#searchOwner").val($("#hiddenOwner").val());
		$("#searchMphone").val($("#hiddenMphone").val());
		$("#searchState").val($("#hiddenState").val());

		$.ajax({
			type:"get",
			dataType:"json",
			url:"workbench/clue/searchClue.do",
			data:{
				"pageNo":pageNo,
				"pageSize":pageSize,
				"fullname":$.trim($("#searchFullname").val()),
				"company":$.trim($("#searchCompany").val()),
				"phone":$.trim($("#searchPhone").val()),
				"source":$.trim($("#searchSource").val()),
				"owner":$.trim($("#searchOwner").val()),
				"mphone":$.trim($("#searchMphone").val()),
			...
			success:function (data){
				...
				$.each(data.dataList,function (i,n){
					...
					html += '	<td><input type="checkbox" name="selectClue" value="'+n.id+'"/></td>';
...
<body>
<input type="hidden" id="hiddenFullname">
...
<input type="hidden" id="hiddenMphone">
				...
				<form class="form-inline" role="form" style="position: relative;top: 8%; left: 5px;">
				  ...
				  <div class="form-group">
				    <div class="input-group">
				      ...
				      <input class="form-control" type="text" id="searchFullname">
				    </div>
				  </div>
				  ...
				  <div class="form-group">
				    <div class="input-group">
				      ...
				      <input class="form-control" type="text" id="searchCompany">
				    </div>
				  </div>
				  ...
				  <div class="form-group">
				    <div class="input-group">
				      ...
				      <input class="form-control" type="text" id="searchMphone">
				    </div>
				  </div>
